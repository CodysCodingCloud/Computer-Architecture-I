...
bool KmapProblem::set_num_vars(int& num_vars) {
    _num_vars = num_vars;
    return true;
};
int KmapProblem::gen_memo_elem(std::vector<int> term) {
    int temp_elem = 0;
...
bool KmapProblem::set_problem(std::vector<std::vector<int>>& newproblem) {
    _problem = newproblem;
    return true;
};
...
bool KmapProblem::simplify_problem(std::vector<std::vector<int>>& problem, std::vector<std::vector<int>> simp_terms, std::set<int> memo, int i_, int j_) {
    try {
        ...
        for (int i = i_; i < problem.size(); i++) {
            ...
            int curr_memo_elem = gen_memo_elem(problem[i]);
            for (int j = used_j ? i + 1 : j_; j < problem.size(); j++) {
                ...
                if (is_gray(problem[i], problem[j])) {
                    ...
                    int temp_dupe = gen_memo_elem(term_simp);
                    if (memo.find(temp_dupe) == memo.end()) {
                        ...
                        memo.insert(temp_dupe);
                        memo.insert(curr_memo_elem);
                        memo.insert(gen_memo_elem(problem[j]));
                        grouped_prev = true;
                        simped = true;
        ...
        if (simped) {
            ...
            std::cout << "more simp needed" << std::endl;
            simplify_problem(simp_terms);
        } else {
            // no more simplification possible
            std::cout << "ans added" << std::endl;
